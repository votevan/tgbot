...
def paste(bot: Bot, update: Update, args: List[str]):
    ...
    if message.reply_to_message:
        ...
        message.reply_text("❌ Qué debería hacer con esto?")
        return

    r = requests.post(f'{BASE_URL}/documents', data=data)

    if r.status_code == 404:
        update.effective_message.reply_text('❌ No hay conexión con dogbin.')
        r.raise_for_status()
    ...
    key = res['key']
    if res['isUrl']:
        reply = f'➡️ URL acortada: {BASE_URL}/{key}\nℹ️ Podés ver estadísticas, etc. [acá]({BASE_URL}/v/{key}).'
    else:
...
def get_paste_content(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        message.reply_text("❌ Tenés que escribir la clave de pegado!")
        return
    ...
    if r.status_code != 200:
        try:
            ...
            if r.status_code == 404:
                ...
                update.effective_message.reply_text('❌ Error desconocido')
        r.raise_for_status()
...
__help__ = """
 - /paste: crear un enlace acortado o pegar algo usando [dogbin](https://del.dog)
 - /getpaste: obtener el contenido de algo de [dogbin](https://del.dog)
 - /pastestats: ver estadisticas de un documento de [dogbin](https://del.dog)
"""
