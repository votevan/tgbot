...
def set_about_me(bot: Bot, update: Update):
    ...
    if len(info) == 2:
        if len(info[1]) < MAX_MESSAGE_LENGTH // 4:
            ...
            message.reply_text("Perfecto.") #Original: Updated your info!
        else:
            message.reply_text(
                "no máquina son al menos {} caracteres y tenés {}.".format(MAX_MESSAGE_LENGTH // 4, len(info[1])))
...
def about_bio(bot: Bot, update: Update, args: List[str]):
    ...
    if info:
        ...
        update.effective_message.reply_text("{} es un sujeto de pocas palabras".format(username))
                                            #Original: {} hasn't had a message set about themselves yet!
    else:
        update.effective_message.reply_text("aún no tenés una bio salame")
...
def set_about_bio(bot: Bot, update: Update):
    ...
    if message.reply_to_message:
        ...
        if user_id == message.from_user.id:
            message.reply_text("no podés establecer tu propia biografía xdd")
                              #Original: Ha, you can't set your own bio! You're at the mercy of others here...
            return
        elif user_id == bot.id and sender.id not in SUDO_USERS:
            message.reply_text("kaces solo confío en los sudo para configurar mi bio")
        ...
        if len(bio) == 2:
            if len(bio[1]) < MAX_MESSAGE_LENGTH // 4:
                ...
                message.reply_text("listo máquina, actualicé la bio de {}".format(repl_message.from_user.first_name))
                                   #Original: Updated {}'s bio!
            else:
                message.reply_text(
                    "necesitas {} caracteres para la bio e intentaste poner {}".format(
                    #Original: A bio needs to be under {} characters! You tried to set {}.
                        MAX_MESSAGE_LENGTH // 4, len(bio[1])))
    else:
        message.reply_text("no master, respondele el mensaje")
...
def __user_info__(user_id):
    ...
    if bio and me:
        return "<b>Acerca de este man:</b>\n{me}\n<b>Lo que dicen:</b>\n{bio}".format(me=me, bio=bio)
               #Original: <b>About user:</b>\n{me}\n<b>What others say:</b>\n{bio}
    elif bio:
        return "<b>Lo que dicen:</b>\n{bio}\n".format(me=me, bio=bio)
...
- /setme <texto>: configurá tu bio
- /me: extraé la bio tuya o de otra persona
"""
